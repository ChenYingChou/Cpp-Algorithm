  ...
  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   分析:
    ...
    ------  ------------------------------------------------------------------
       ...
       8    11: 1111 1111 --+
                 1112111    |
                 112 211    |    把最外層1拿掉, 就是6的全部組合(大於等於1開頭者)
                  11411     |--> 1 (6) 1 => C(6) = 7
                  12221     |
                  13 31     |
                   161    --+

                  22 22   --+    把最外層2拿掉, 就是4組合中大於等於2開頭者
    ...
    若為節省空間, 括號或減號後的數值可不儲存, 即對 N 而言儲存到 N/3 即可。
    因為一對(可被2整除)及本身的計數均為1, 沒儲存的必要。
...
ULL UNI_PDD::operator() (const int N, const int index)
{
    ...
    // 確保有足夠的空間儲存, 0~2 位置均不使用
    if (N >= _state.size()) _state.resize(N+1, nullptr);

    vULL *p = _state[N];
    if (p) return (*p)[index];  // 已計算有值, 直接返回之前計算結果

    p = new vULL(maxCol+1, 0);  // 第一個位置 C(N, 0) =  ΣC(N, 1~N)
    _state[N] = p;

    vULL &X = *p;

    // 計算 C(N, 0) = C(N, 1~N/3)
    ULL sum = 2 - (N & 1);      // 本身 + 是否為一對(被2整除) = 2 - (odd ? 1 : 0)
    if (debug > 1) {
        cout << N << ": sum = " << sum << endl;
    }

    int N2 = N - 2;
    sum += X[1] = operator()(N2);           // C(N-2): 1 (N-2) 1
    if (debug > 1) {
        cout << N << ": sum = " << sum
            << " after plus C(" << N2 << ")="
            << X[1] << endl;
