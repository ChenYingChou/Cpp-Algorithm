...
class Weapon {
  ...
  public:
    ...
    virtual string report() const { return types_name(); }
};
  ...
  public:
    ...
    virtual int attack(Warrior *opponent) {
        ...
        if (opponent->elements(-atk1) > 0) {
            ...
            return elements(-atk2) <= 0 ? -1 : 0;       // -1:自己陣亡, 0:雙方沒死
        } else {
            // 對手為獅子死亡時將其生命值移轉到自己
...
class Warrior_Dragon: public Warrior {
  public:
    Warrior_Dragon(HeadQuarter *hq, int id, int elements, double morale)
    : Warrior(hq, id, _dragon_, elements) {
        // 一件武器
...
class Warrior_Ninja: public Warrior {
  public:
    Warrior_Ninja(HeadQuarter *hq, int id, int elements)
    : Warrior(hq, id, _ninja_, elements) {
        set_fightback(false);
        // 兩件武器
        add_weapon(weapon_gen(this, id));
...
class Warrior_Iceman: public Warrior {
  private:
    int _count;
  public:
    Warrior_Iceman(HeadQuarter *hq, int id, int elements)
    : Warrior(hq, id, _iceman_, elements) {
        // 一件武器
        add_weapon(weapon_gen(this, id));
        _count = 0;
    }
    int forward(int dir) {
        if (city() != target()) {
            ...
            if (_count >= 2) {
                _count = 0;
                if (elements(-9) <= 0) set_elements(1);
                set_atk(atk()+20);
            }
        }
        return Warrior::forward(dir);
    }
};
...
class Warrior_Lion: public Warrior {
  public:
    Warrior_Lion(HeadQuarter *hq, int id, int elements, int loyalty)
    : Warrior(hq, id, _lion_, elements) {
...
class Warrior_Wolf: public Warrior {
  public:
    Warrior_Wolf(HeadQuarter *hq, int id, int elements)
    : Warrior(hq, id, _wolf_, elements) {
        // 無特點
    }
...
int HeadQuarter::generate()
{
    ...
    switch(wid) {
      case _dragon_:
        // 士氣: 为它降生后其司令部剩余生命元的数量除以造dragon所需的生命元数量
        pw = new Warrior_Dragon(this, _count, nElem, (double)_elements/nElem);
        break;
      case _ninja_:
        pw = new Warrior_Ninja(this, _count, nElem);
        break;
      case _iceman_:
        pw = new Warrior_Iceman(this, _count, nElem);
        break;
      case _lion_:
        // 忠诚度: 等于它降生后其司令部剩余生命元的数目
        pw = new Warrior_Lion(this, _count, nElem, _elements);
        break;
      case _wolf_:
        pw = new Warrior_Wolf(this, _count, nElem);
        break;
    }
